...
Function Select-Context() {
    ...
    Write-Host "cc test inside Select-Context()"
    Write-Host "environmentName is '$($environmentName)'"
    Write-Host "context is '$($context)'"

    # $environmentName = $script:EnvironmentName 
    # Write-Host "new environmentName is '$($environmentName)'"
    ...
    Write-Host "script:tenantId here is '$($script:tenantId)' "
    ...
    try {
        ...
        Write-Host "azProfile.Context is '$($azProfile.Context)' "
        ...
        try {
            # cc add
            # for the case where prompting web browser is not applicable, we will prompt for another login.
            $azProfile = Connect-AzAccount `
...
Function New-ADApplications() {
    ...
    try {
        Write-Host "cc test - inside New-ADApplications() "
        ...
        try {
            Write-Host "cc test Connect-AzureAD -1"
            $creds = Connect-AzureADAlias -AzureEnvironmentName $context.Environment.Name -TenantId $tenantId -AccountId $context.Account.Id -Credential $context.Account.Credential
        }
        catch {
            ...
            try {
                Write-Host "cc test Connect-AzureAD -2"
                $creds = Connect-AzureADAlias -AzureEnvironmentName $context.Environment.Name -TenantId $tenantId
                }
            catch {
        ...
        Write-Host "creds is '$($creds)'"
        if (!$creds) {
            #Write-Host "cc test 3"
            return $null
        }
...
# For CloudShell
$isCloudShell = $false
Write-Host "cc test - beginning"
Write-Host "TenantId is '$($TenantId)'"
#Connect-AzAccount -UseDeviceAuthentication -TenantId fb9c1461-457f-4e6f-bf92-3f211043a490
if (Get-Module -ListAvailable -Name "AzureAD.Standard.Preview") {
    ...
    #Set-Alias -Name Connect-AzureADAlias -Value "Connect-AzureAD"
    Write-Host "cc test - Set-Alias"
...
Write-Host "cc test 0"
Write-Host "script:Context is '$($script:Context)'"  
Write-Host "Name is '$($script:Name)'"    
Write-Host "script:EnvironmentName is '$($script:EnvironmentName)'"   

$selectedContext = Select-Context -context $script:Context -environmentName $script:EnvironmentName

Write-Host "cc test 1"
Write-Host "selectedContext is '$($selectedContext)'"
Write-Host "script:Name is '$($script:Name)'"

$aadConfig = New-ADApplications -applicationName $script:Name -context $selectedContext

Write-Host "cc test 2"
