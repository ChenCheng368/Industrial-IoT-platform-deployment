...
Function Select-Context() {
    ...
    if (!$context) {
        try {
            ...
            $connection = Connect-AzAccount -UseDeviceAuthentication -Environment $environment.Name `
...
Function New-Deployment() {
    ...
    if (!$script:aadConfig) {
        ...
        if (![string]::IsNullOrEmpty($authTenantId)) {
            ...
            Connect-AzAccount -UseDeviceAuthentication -Tenant $authTenantId -ContextName AuthTenantId -Force
